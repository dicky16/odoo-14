...
def _generate_assessment_note_ids(cr, registry):
    ...
    # default_employee_mail_template = env['res.company']._get_default_appraisal_confirm_employee_mail_template()
    # default_manager_mail_template = env['res.company']._get_default_appraisal_confirm_manager_mail_template()
    ...
    env['res.company'].search([]).write({
        ...
        # 'appraisal_confirm_employee_mail_template': default_employee_mail_template.id,
        # 'appraisal_confirm_manager_mail_template': default_manager_mail_template.id,
    })

    env['hr.job'].search([]).write({
